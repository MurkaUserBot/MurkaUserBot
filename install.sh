...
if [ ! -n "$BASH" ]; then
  ...
  bash -c '. <('"$(command -v curl >/dev/null && echo 'curl -Ls' || echo 'wget -qO-')"' https://raw.githubusercontent.com/MurkaUserBot/MurkaUserBot/master/install.sh) '"$*"
  exit $?
fi
...
banner() {
  ...
  echo "|  \\/  |          | |       | |  | |             |  _ \\      | |  "
  echo "| \\  / |_   _ _ __| | ____ _| |  | |___  ___ _ __| |_) | ___ | |_ "
  echo "| |\\/| | | | | \'__| |/ / _\` | |  | / __|/ _ \\ \'__|  _ < / _ \\| __|"
  echo "| |  | | |_| | |  |   < (_| | |__| \\__ \\  __/ |  | |_) | (_) | |_  "
  echo "|_|  |_|\\__,_|_|  |_|\\_\\__,_|\\____/|___/\\___|_|  |____/ \\___/ \\__|"
}
